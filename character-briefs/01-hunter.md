# The Hunter, character brief

**1 of 8. The pilot.** Do this one first, all the way to a moving rig, before anybody starts a
second. Freestyle - Outcome - Solo. "Give it a target and get out of its way."

**Palette: `#FF8C42` main, `#7A2E0E` deep. Nothing else appears on this character.**

## What makes this one the Hunter

Distinguishing feature: the **visor**. A narrow horizontal slit rather than the full
oval the others wear, set into a head that sits slightly forward of the shoulders. Trim, no
spare armour, built to move. It should read as already looking at something.

The current render is the reason for this whole round. The Hunter is an orange type, in an
orange scene, with **cyan eyes**. Delete the background and there is very little Hunter left.
...
> boots, a small V chevron on the chest. Narrow horizontal visor slit, head set slightly
> forward of the shoulders, trim frame with minimal armour. Accent panels and glowing eyes in
> warm orange #FF8C42, with deep burnt umber #7A2E0E in the recesses, no other hues. Soft key
...
| File | Contains |
|---|---|
| `hunter-shadow.png` | Contact shadow only, black on alpha |
| `hunter-body.png` | Torso, hips, legs, boots. No arms, no head |
| `hunter-arm-l.png` | Shoulder to fingertip, cut at the joint |
| `hunter-arm-r.png` | As above |
| `hunter-head.png` | Helmet shell. **Visor cut out to transparent** |
| `hunter-visor.png` | The dark glass only, no eyes |
| `hunter-glow.png` | Eyes and chest mark, **drawn in flat white** |

**The glow layer is white, not warm orange.** The app tints it with the user's own colour at
...
2. Beside the shipped Hunter, is it recognisably the same character?
...
**the visor slit narrows to a line, then snaps wide on the release**.

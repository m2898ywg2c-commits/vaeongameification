    ...
    <main className="min-h-screen text-white px-5 py-8" style={{ background: "linear-gradient(180deg, " + deep + "33 0%, #0E1224 42%)" }}>
      <div className="max-w-md mx-auto">
        ...
        <a href="/plan" className="block rounded-2xl p-5 mb-3" style={{ background: "linear-gradient(135deg, " + accent + ", " + deep + ")", color: "#fff" }}>
          <p className="text-xs font-bold uppercase tracking-wide opacity-75">{profile.fixed_days === false ? "Next up" : today}</p>
          <p className="text-2xl font-bold leading-tight">Todays workout</p>
          <p className="text-sm font-medium opacity-85 mt-1">Tap to open and start logging</p>
        </a>
        ...
        {type ? (
          <div className="rounded-2xl p-4 mb-3 border" style={{ borderColor: accent + "55", background: deep + "33" }}>
            <div className="flex items-center gap-2 mb-2">
              <TypeOrb typeId={typeId} size={26} />
              <p className="text-xs font-bold uppercase tracking-wide" style={{ color: accent }}>{type.name} says</p>
            </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 mb-5">
          <div className="flex items-center justify-between mb-1">
            <p className="text-sm font-bold">Block {profile.block_number || 1} · Week {weekNo}/{BLOCK_WEEKS}</p>
          ...
          {finished ? <a href="/settings" className="text-xs underline block mt-2" style={{ color: "#3DDC97" }}>Block complete. Start the next one.</a> : null}
        ...
        <a href="/onboarding" className="block text-center text-xs text-gray-500 underline">Change goals or sessions a week</a>
      </div>
    </main>
  );
}

...
// not arise.
export const LIGHT_DAY_LOAD = 0.80;

export function workingSets(exerciseName, profile, weekPct, maxes, totalSets, intensity) {
  const base = workingWeight(exerciseName, profile, weekPct, maxes);
  if (!base) return null;
...
export function blockProjection(exerciseName, profile, maxes, totalSets, weeks, intensity) {
  const ladder = weeks && weeks.length ? weeks : LIFT_WEEKS;
  const out = ladder.map(function (w, i) {
    const sets = workingSets(exerciseName, profile, w.pct, maxes, totalSets, intensity);
    return {
      week: i + 1,
      pct: w.pct,

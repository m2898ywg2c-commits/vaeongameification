...
export function workingSets(exerciseName, profile, weekPct, maxes, totalSets, intensity, prescribedReps, floor) {
  ...
  if (f > 0) {
    const openingTop = workingWeight(exerciseName, profile, OPENING_PCT, maxes, prescribedReps);
...
export function blockProjection(exerciseName, profile, maxes, totalSets, weeks, intensity, prescribedReps, floor) {
  const ladder = weeks && weeks.length ? weeks : LIFT_WEEKS;
  const out = ladder.map(function (w, i) {
    const sets = workingSets(exerciseName, profile, w.pct, maxes, totalSets, intensity, prescribedReps, floor);
    return {
      week: i + 1,
      pct: w.pct,

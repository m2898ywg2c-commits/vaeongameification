...
//
// Now 0.88 at the deload to 1.08 at week six. Week one lands just under demonstrated
// capacity, which is a start rather than a step back, and the block gains about eight percent.
// That is ambitious for a true intermediate and about right for a first structured block. The
// safety net is already in the copy on every card: if a lift stalls, hold the weight and add
// a rep rather than forcing the number, and the end of week review flags anything reading hard.
export function weekFactor(weekPct) {
  const p = Number(weekPct) || 0.70;
  return 0.88 + ((p - 0.65) / (0.90 - 0.65)) * 0.20;
}
...
export function floorFromHistory(loggedSets, prescribedReps) {
  ...
  loggedSets.forEach(function (row) {
    ...
    // reps >= r is the test: doing it for MORE reps proves at least this much for fewer.
    if (w > floor && reps >= r) floor = w;
  });

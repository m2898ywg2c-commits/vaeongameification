...
// not arise.
// WAS 0.80, AND THAT WAS DOING A JOB IT NO LONGER HAS.
//
// The flat twenty percent cut existed to make the volume day easier than the heavy day back
// when both were prescribed from the same rep-blind number. Now that the load is derived
// from each day's own rep count, the volume day is already lighter for the right reason: it
// asks for more reps. Stacking the old cut on top made week six of the volume day land below
// what James was lifting in week one, which is how this got reported.
//
// A small trim is still right. A volume day should be submaximal for its rep count, not a
// second attempt at a personal best.
export const LIGHT_DAY_LOAD = 0.95;

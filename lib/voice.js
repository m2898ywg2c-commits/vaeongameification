...
// IT WAS SEEDED ON THE DAY OF THE MONTH, SO IT NEVER CHANGED.
//
// (dayOfMonth + tabIndex) % 6 is fixed for a whole day. Open the app four times on a Tuesday
// and you got the same line four times, which is the fastest way to make a motivational quote
// read as wallpaper.
//
// `rotation` is a counter the caller bumps on each visit, so the list advances one step every
// time the Ready card appears. Cycling rather than picking at random on purpose: with six
// quotes, random gives you a repeat back to back about one time in six, and a repeat is
// exactly the thing being fixed. Cycling shows all six before any of them comes round again.
export function quoteFor(typeId, seed, rotation) {
  const list = QUOTES[typeId] || QUOTES.architect;
  const r = Number(rotation);
  const base = Number.isFinite(r) ? r : new Date().getDate();
  const i = (((base + (seed || 0)) % list.length) + list.length) % list.length;
  return list[i];
}

// Bump and read the rotation counter. Kept here next to the quotes so the storage key and the
// list cannot drift apart, and wrapped because localStorage throws in private browsing on iOS,
// which is a real configuration for this user base.
const ROTATION_KEY = "vaeon-quote-rotation";

export function nextQuoteRotation() {
  try {
    const n = Number(window.localStorage.getItem(ROTATION_KEY)) || 0;
    const next = (n + 1) % 1000000;
    window.localStorage.setItem(ROTATION_KEY, String(next));
    return next;
  } catch (e) {
    // No storage: fall back to something that at least varies within a session.
...
const PRAISE = {
  architect: ["Logged.", "Set recorded.", "On plan.", "Exactly as written.", "Noted. Next."],
  captain: ["That is it!", "Yes! Again.", "Outstanding.", "Leading from the front.", "Get in!"],
  monk: ["Good.", "One more, quietly.", "Steady.", "That was clean.", "Well held."],
  anchor: ["Nicely done.", "That is another one banked.", "Lovely.", "Good work.", "Steady as ever."],
  hunter: ["Down.", "Got it.", "Next target.", "Clean kill.", "Move."],
  gladiator: ["YES!", "Have that!", "Absolute weapon.", "Again! Again!", "That is the one!"],
  wanderer: ["Nice one.", "That felt alright, didnt it.", "Onwards.", "Good stuff.", "Keep drifting."],
  spark: ["Yesss!", "Love that!", "Boom.", "You are on fire.", "Brilliant!"],
};

export function praiseFor(typeId, n) {
  const list = PRAISE[typeId] || PRAISE.architect;
  return list[(n || 0) % list.length];
}

const SESSION_DONE = {
  architect: "Session complete. The data is what matters, and the data looks right.",
  captain: "Session done. You set the standard today. Well led.",
  monk: "Complete. Nothing rushed, nothing wasted. Rest now.",
  anchor: "Done and dusted. You turned up, which is most of it.",
  hunter: "Target down. Log it and go.",
  gladiator: "WORK. That is how you win a block.",
  wanderer: "Session done. Hope that felt good.",
  spark: "Smashed it! Go and tell someone.",
};

export function sessionDone(typeId) {
  return SESSION_DONE[typeId] || SESSION_DONE.architect;
}

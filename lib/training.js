...
// HOW EACH ONE IS LOGGED, WHICH IS NOT THE SAME QUESTION FOR ALL FIVE.
//
// A SkiErg or a Row is a number you chase: one total time, compared week on week. Battle
// ropes and a sled are not. Nobody times ten thirty-second rope intervals and nobody beats
// their own sled time; you either got through the rounds or you did not. Asking for "your
// time or score" on those produced an empty box that people sensibly ignored.
//
// log: "time"   one figure, typed, meant to be beaten.
// log: "rounds" tick them off as you finish them. rounds is how many there are.
const HYROX_STATIONS = [
  { name: "SkiErg", target: "3 x 500m, 90 sec recovery", note: "Log your total time. This is what you chase from here.", log: "time" },
  { name: "Row", target: "3 x 500m, 90 sec recovery", note: "Strong legs first, then body, then arms.", log: "time" },
  // Slam balls rather than battle ropes. Ropes are the one station in this list that a
  // normal commercial gym often does not have, and prescribing kit somebody cannot reach
  // makes the whole finisher feel optional. A slam ball trains the same thing: repeated
  // full-body output with no eccentric to recover from.
  { name: "Slam Balls", target: "10 x 30 sec on / 30 sec off", note: "Hips through, catch it low, straight back up.", log: "rounds", rounds: 10 },
  // Logged as distance, not rounds, because the prescription is already in metres and the
  // number worth beating is how far you moved the sled, not how many times you walked it.
  // Rounds made it the odd one out: SkiErg and Row give you a number to chase and this
  // gave you five ticks that said nothing about the load or the floor.
  { name: "Sled Push & Pull", target: "4 rounds: 20m push, 20m pull", note: "Low body angle, drive through the legs. Log total metres moved.", log: "distance", target_m: 160 },
  { name: "Weighted Lunge Walk", target: "5 x 100m, walk back to recover", note: "Torso upright, controlled stride.", log: "rounds", rounds: 5 },
];
...
// WHICH WEEKDAYS THE PLAN LANDS ON.
//
// SPREAD is the fallback, not the answer. It used to be the only thing here, which meant
// buildWeek() never once looked at the days the user actually chose. Somebody picking
// Monday to Friday got SPREAD[5], which is [0,1,2,4,5], which is Mon Tue Wed Fri Sat. Their
// Thursday vanished and they gained a Saturday they never asked for. The choice was written
// to profiles.train_days in onboarding, read back into the settings screen so it looked
// honoured, and then discarded at the point it mattered.
//
// INDEX CONVENTIONS, WHICH ARE NOT THE SAME AND THAT IS THE TRAP.
//
// profiles.train_days is 0 = Sunday, matching JavaScript's getDay(). DAY_LABELS is
// 0 = Monday, because a training week reads Monday first. Converting is (d + 6) % 7 and
// forgetting it shifts every session by a day.
export function slotsFor(n, trainDays, fixedDays) {
  // fixed_days === false means numbered sessions rather than weekdays, so the spread is
  // correct for them: there is no chosen day to honour.
  if (fixedDays !== false && Array.isArray(trainDays) && trainDays.length) {
    const converted = trainDays
      .map(function (d) { return (Number(d) + 6) % 7; })
      .filter(function (d) { return d >= 0 && d <= 6; })
      .sort(function (a, b) { return a - b; });
    const unique = converted.filter(function (d, i) { return converted.indexOf(d) === i; });
    if (unique.length >= n) return unique.slice(0, n);
    // Picked fewer days than sessions pledged. Keep every day they chose and top up from
    // the spread rather than silently dropping sessions.
    const fill = (SPREAD[n] || SPREAD[3]).filter(function (d) { return unique.indexOf(d) === -1; });
    // Sorted again after topping up, or the week renders Mon Wed Tue Thu.
    return unique.concat(fill).slice(0, n).sort(function (a, b) { return a - b; });
  }
  return SPREAD[n] || SPREAD[3];
}

// ---------- Variance across the week ----------
//
// TWO GOALS USED TO MEAN TWO NEARLY IDENTICAL LEG DAYS, BACK TO BACK.
//
// The interleave below picks primary.days[0..1] and secondary.days[0..1] and knows nothing
// about what is inside them. For hyrox plus strength at four sessions that produced:
//
//   Tue hyrox-push   Bench 5x5, Incline DB, Weighted Dips, Push Press, Fly, Leg Raise, Plank
//   Wed st-squat     Back Squat 5x5, Front Squat, RDL, Leg Press, Plank
//   Thu hyrox-legs   Back Squat 5x5, RDL, Lunges, Sled, Hip Thrust, Leg Press, Woodchop
//   Fri st-bench     Bench 5x5, OHP, Weighted Dips, Barbell Row, Pushdown
//
// Three of Wednesday's five exercises reappeared on Thursday, including a five by five back
// squat on consecutive days. That is not variety missing, it is a recovery problem: heavy
// squats twice in twenty four hours is how somebody picks up a back injury in week three.
//
// Two passes fix it. Order the days so the least similar sessions sit next to each other,
// then substitute whatever still repeats. Substitution is by movement pattern, so the
// session keeps doing the job it was written to do.
const SUBSTITUTES = {
  "back squat": ["Front Squat", "Hack Squat", "Goblet Squat", "Box Squat"],
  "front squat": ["Back Squat", "Goblet Squat", "Hack Squat"],
  "romanian deadlift": ["Good Morning", "Single Leg RDL", "Back Extension", "Nordic Curl"],
  "deadlift": ["Trap Bar Deadlift", "Rack Pull", "Romanian Deadlift"],
  "leg press": ["Hack Squat", "Split Squat", "Step Up", "Belt Squat"],
  "leg curl": ["Nordic Curl", "Single Leg RDL", "Good Morning"],
  "leg extension": ["Sissy Squat", "Step Up", "Split Squat"],
  ...
  "barbell bench press": ["Dumbbell Bench Press", "Incline Barbell Press", "Floor Press"],
  "dumbbell bench press": ["Barbell Bench Press", "Incline Dumbbell Press", "Floor Press"],
  "incline barbell press": ["Incline Dumbbell Press", "Barbell Bench Press", "Landmine Press"],
  "incline dumbbell press": ["Incline Barbell Press", "Dumbbell Bench Press"],
  "overhead press": ["Seated Dumbbell Press", "Push Press", "Landmine Press"],
  "weighted dips": ["Close Grip Bench Press", "Tricep Pushdown", "Ring Dips"],
  "cable fly": ["Dumbbell Fly", "Pec Deck", "Cable Crossover"],
  "barbell row": ["Chest Supported Row", "Single Arm DB Row", "Cable Row"],
  "cable row": ["Barbell Row", "Chest Supported Row", "Single Arm DB Row"],
  "pull ups": ["Chin Ups", "Lat Pulldown", "Neutral Grip Pull Ups"],
  "chin ups": ["Pull Ups", "Lat Pulldown", "Neutral Grip Pull Ups"],
  "barbell curl": ["EZ Bar Curl", "Incline DB Curl", "Cable Curl"],
  "hammer curl": ["Rope Curl", "Reverse Curl", "Incline DB Curl"],
  "plank": ["Hollow Hold", "Dead Bug", "Side Plank", "Ab Wheel"],
  "ab wheel": ["Hollow Hold", "Plank", "Cable Crunch"],
  "cable crunch": ["Hanging Leg Raise", "Ab Wheel", "Dead Bug"],
  "hanging leg raise": ["Cable Crunch", "Toes to Bar", "Reverse Crunch"],
  "cable woodchop": ["Pallof Press", "Russian Twist", "Landmine Rotation"],
  "russian twist": ["Pallof Press", "Cable Woodchop", "Side Plank"],
  "farmers carry": ["Suitcase Carry", "Front Rack Carry", "Sandbag Carry"],
  "sled push": ["Sled Drag", "Prowler Push", "Hill Sprint"],
  "weighted sled push": ["Sled Drag", "Prowler Push", "Heavy Sandbag Carry"],
  "weighted walking lunges": ["Reverse Lunge", "Step Up", "Bulgarian Split Squat"],
  ...
  "face pull": ["Reverse Fly", "Band Pull-Apart", "Rear Delt Row"],
  "row": ["SkiErg", "Bike Intervals", "Assault Bike"],
  "wall balls": ["Thrusters", "Slam Balls", "Goblet Squat to Press"],
  // Single goal plans keep their authored day order, so substitution is the only lever
  // available to them. These are the ones that were still landing on back to back days:
  // run5k puts Cool Down Jog on both intervals and tempo, and yoga puts Chair Pose in both
  // Foundations and Balance.
  "cool down jog": ["Easy Walk Back", "Cool Down Walk", "Easy Spin"],
  "chair pose": ["Goddess Pose", "High Lunge", "Horse Stance"],
  "post-run stretch": ["Hip Flexor Stretch", "Calf and Hamstring Stretch"],
  "plank hold": ["Side Plank Hold", "Forearm Plank", "Hollow Hold"],
  "downward dog": ["Dolphin Pose", "Puppy Pose"],
  "warrior ii": ["Extended Side Angle", "Reverse Warrior"],
  "child's pose": ["Supine Twist", "Happy Baby"],
  "easy run": ["Easy Trail Run", "Recovery Jog"],
  "tempo run": ["Progression Run", "Cruise Intervals"],
  "goblet squat": ["Front Squat", "Box Squat", "Split Squat"],
  "seated row": ["Cable Row", "Chest Supported Row", "Single Arm DB Row"],
  "push ups": ["Incline Push Ups", "Diamond Push Ups", "Ring Push Ups"],
  "brisk walk or incline treadmill": ["Steady Cycle", "Cross Trainer Steady"],
  "lat pulldown": ["Pull Ups", "Chin Ups", "Straight Arm Pulldown", "Neutral Grip Pulldown"],
  "long run": ["Long Trail Run", "Long Steady Run"],
  "tricep rope pushdown": ["Skull Crusher", "Overhead Tricep Extension", "Close Grip Bench Press"],
  "dumbbell shoulder press": ["Arnold Press", "Landmine Press", "Overhead Press"],
  "double db push press": ["Push Jerk", "Overhead Press", "Landmine Press"],
  "preacher curl": ["Concentration Curl", "Spider Curl", "Cable Curl"],
  "tree pose": ["Eagle Pose", "Half Moon", "Standing Figure Four"],
  "warrior iii": ["Half Moon", "Airplane Pose", "Standing Split"],
  "low lunge": ["Crescent Lunge", "Lizard Pose", "Dragon Pose"],
  "legs up the wall": ["Supine Twist", "Reclined Butterfly", "Savasana"],
  "tempo block": ["Threshold Block", "Cruise Block"],
  "marathon pace block": ["Steady State Block", "Race Pace Block"],
};

function exNames(day) {
  return (day && day.exercises ? day.exercises : []).map(function (ex) {
    return String(ex.name || "").toLowerCase();
  });
}

// How much two days tread on each other, as a share of the smaller day.
function overlapScore(a, b) {
  const an = exNames(a);
  const bn = exNames(b);
  if (!an.length || !bn.length) return 0;
  let shared = 0;
  an.forEach(function (n) { if (bn.indexOf(n) !== -1) shared++; });
  return shared / Math.min(an.length, bn.length);
}

// Greedy reorder: keep the first session where it is, then repeatedly take whichever
// remaining session overlaps least with the one just placed. Greedy rather than exhaustive
// because six sessions is 720 permutations and this runs on every plan render.
function spreadDays(days) {
  ...
  while (rest.length) {
    ...
    rest.forEach(function (d2, i) {
      const s = overlapScore(prev, d2);
...
// Replace anything that still repeats. Yesterday's exercises are the hard constraint, since
// that is the recovery question. Anything appearing three or more times in a week is also
// thinned, because at that point it is the plan running out of ideas rather than an
// intentional emphasis.
//
// A substitution keeps sets, reps and note, so the prescription and the progression are
// untouched. Only the name moves, which matters because lift_maxes is keyed on the name:
// a swapped exercise starts its own history rather than inheriting the wrong one.
function varyExercises(days) {
  const weekCount = {};
  days.forEach(function (d2) {
    exNames(d2).forEach(function (n) { weekCount[n] = (weekCount[n] || 0) + 1; });
  });

  let prevNames = [];
  return days.map(function (d2) {
    const usedToday = [];
    const exercises = (d2.exercises || []).map(function (ex) {
      const name = String(ex.name || "").toLowerCase();
      const clashesWithYesterday = prevNames.indexOf(name) !== -1;
      const overusedThisWeek = (weekCount[name] || 0) > 2;
      if (!clashesWithYesterday && !overusedThisWeek) {
        usedToday.push(name);
        return ex;
      }
      const options = SUBSTITUTES[name] || [];
      const pick = options.find(function (alt) {
        const a = alt.toLowerCase();
        return prevNames.indexOf(a) === -1 && usedToday.indexOf(a) === -1 && !weekCount[a];
      });
      // No clean alternative means the original stands. A repeated exercise is a smaller
      // problem than a session quietly losing a movement pattern.
      if (!pick) { usedToday.push(name); return ex; }
      usedToday.push(pick.toLowerCase());
      weekCount[name] = Math.max(0, (weekCount[name] || 1) - 1);
      weekCount[pick.toLowerCase()] = (weekCount[pick.toLowerCase()] || 0) + 1;
      return Object.assign({}, ex, { name: pick });
    });
    prevNames = usedToday;
    return Object.assign({}, d2, { exercises: exercises });
  });
}

// A LIFT TWICE IN A WEEK SHOULD NOT BE THE SAME SESSION TWICE.
//
// After the variance pass, James's week still put Back Squat, Romanian Deadlift and Leg
// Press on both Wednesday and Friday with an identical prescription. Squatting twice a week
// is good programming. Squatting the same weight for the same reps twice is not: it is one
// session run twice, and the second one is where people get flat and bored.
//
// Every serious programme answers this the same way. One day is heavy and lower rep, the
// other carries the volume at a lighter load. Two different stimuli from one movement, and
// crucially the light day is easier, so the week gets harder without the fatigue doubling.
//
// The heavy day is the one prescribing the most sets, ties going to whichever comes first.
// That usually lands the heavy day on the session the lift actually belongs to.
//
// Only bare rep counts are adjusted. "12 per leg", "45 sec" and "20m" are left exactly as
// written, because adding three to a timed hold or a sled distance is meaningless.
// The load side of this lives in lib/progression.js as LIGHT_DAY_LOAD, because that is where
// every other weight decision is made. This file only tags the day and adjusts the reps, so
// it stays free of imports and can be reasoned about on its own.
// Only a bare rep count can be split. "45 sec", "20m" and "12 per side" are a hold, a
// distance and a unilateral set: none of them have a heavy and a light version in the sense
// this is describing, and a plank labelled "heavy day" is just wrong.
function splittable(ex) {
  return /^\d+$/.test(String(ex && ex.reps || "").trim());
}

// Rep counts people actually write on a programme. +3 reps landed on elevens and thirteens,
// which look like a bug even when the arithmetic is right.
const TIDY_REPS = [6, 8, 10, 12, 15];

function lighterReps(reps) {
  const n = Number(String(reps || "").trim());
  if (!n) return null;
  const target = Math.min(15, n + 3);
  let best = TIDY_REPS[0];
  TIDY_REPS.forEach(function (t) {
    if (Math.abs(t - target) < Math.abs(best - target)) best = t;
  });
  // Never prescribe fewer reps on the volume day than on the heavy one.
  return String(Math.max(best, n));
}

function assignIntensity(days) {
  const count = {};
  const bestSets = {};
  days.forEach(function (d) {
    (d.exercises || []).forEach(function (ex) {
      ...
      if (bestSets[k] === undefined || s > bestSets[k]) bestSets[k] = s;
    });
  });
  ...
  return days.map(function (d) {
    return Object.assign({}, d, {
      exercises: (d.exercises || []).map(function (ex) {
        const k = String(ex.name || "").toLowerCase();
        if ((count[k] || 0) < 2) return ex;
        if (!splittable(ex)) return ex;

        const isTopSets = (Number(ex.sets) || 1) === bestSets[k];
        if (!usedHeavy[k] && isTopSets) {
          usedHeavy[k] = true;
          return Object.assign({}, ex, { intensity: "heavy" });
        }
        const lighter = lighterReps(ex.reps);
        if (!lighter) return Object.assign({}, ex, { intensity: "light" });
        return Object.assign({}, ex, {
          intensity: "light",
          reps: lighter,
          note: "Volume day. Same movement, lighter bar, more reps. Leave the heavy work for the other session.",
        });
      }),
    });
  });
...
export function buildWeek(goalIds, sessionsPerWeek, trainDays, fixedDays, weekNo) {
  ...
  // Reorder only when two goals are in play. A single goal's day list was authored in an
  // order that means something (easy, tempo, long for a runner) and shuffling it would be
  // the app second-guessing the programme. Two goals interleaved have no authored order at
  // all, which is exactly where back to back leg days came from.
  if (secondary) picked = spreadDays(picked);
  picked = varyExercises(picked);
  // After substitution, so it only splits what genuinely survived as a repeat.
  picked = assignIntensity(picked);

  // TWO TABS CALLED THE SAME THING IS NOT A WEEK, IT IS A GUESS.
  //
  // Two goals can each own a day called "Speed Intervals" or "Easy Run". The keys differ so
  // the logging was always correct, but the user saw two identical tabs and had no way to
  // tell which one they had already done. Numbered only where it actually collides, so a
  // week with no clash reads exactly as it did before.
  const titleCount = {};
  picked.forEach(function (d2) { titleCount[d2.title] = (titleCount[d2.title] || 0) + 1; });
  const seenTitle = {};
  picked = picked.map(function (d2) {
    if (titleCount[d2.title] < 2) return d2;
    seenTitle[d2.title] = (seenTitle[d2.title] || 0) + 1;
    return Object.assign({}, d2, { title: d2.title + " " + seenTitle[d2.title] });
  });
  ...
  // THE POOL ROTATES ACROSS WEEKS, NOT JUST ACROSS A WEEK.
  //
  // dealt started at zero on every build, so a four session week with five stations dealt
  // the first four every single week and the fifth was unreachable for the entire block.
  // Offsetting by the week number walks the whole pool: four sessions against five stations
  // gives Ski, Row, Ropes, Sled in week one and Row, Ropes, Sled, Lunge Walk in week two.
  //
  // James asked for weighted lunges "on following weeks instead of battle ropes". They were
  // already written; nobody could ever get to them.
  let dealt = Math.max(0, (Number(weekNo) || 1) - 1);

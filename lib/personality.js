...
export function scoreAnswers(values) {
  ...
  // Centre each answer on the respondent's own mean, not the fixed midpoint of 3. This
  // strips out acquiescence bias, the tendency to agree with everything. Without it,
  // agreeable answers net every dimension toward zero and everyone lands on the same
  // all-positive type (the Captain), which makes the quiz feel like it never discriminates.
  let sum = 0;
  let n = 0;
  QUESTIONS.forEach(function (q, i) {
    if (values[i] != null) { sum += values[i]; n += 1; }
  });
  const mean = n ? sum / n : 3;

  QUESTIONS.forEach(function (q, i) {
    const centred = (values[i] != null ? values[i] : mean) - mean;
...
function pickType(t) {
  // Strictly greater than zero, so a dead-even dimension breaks toward the freestyle,
  // experience, solo poles rather than piling everyone onto the all-positive Captain corner.

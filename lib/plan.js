...
import { startOfWeek as startOfWeekShared, sessionsByWeek } from "./week";
...
export function computeStats(sessions, target, frozenWeeks) {
  ...
  // Distinct sessions per week, not rows. The same plan day completed twice in a week is one
  // session towards the pledge, which is what the leaderboard and the progress chart now say
  // too. See sessionKey in lib/week.js for why, and supabase/2026-08-09_session_key.sql for
  // the matching definition in Postgres.
  const counts = sessionsByWeek(list);
  const thisWeekCount = counts[thisWeek] || 0;
  const kept = function (wk) {
    return (counts[wk] || 0) >= goal || Boolean(frozen[wk]);
  };

  // WALKED BACK IN DAYS, NOT IN MILLISECONDS.
  //
  // This subtracted a fixed 604800000ms per step. In the UK the week containing the last
  // Sunday in October is 25 hours long, so from that point every key in the walk is an hour
  // adrift of the keys the sessions are filed under, `kept` returns false against weeks that
  // were kept, and a long streak silently truncates at the clock change. Re-deriving the
  // boundary from a stepped date cannot drift.
  const backWeeks = function (n) {
